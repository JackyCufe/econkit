...
# ─── T30: build_regression_table（5模型并排三线表）─────────────────────────
print('\n30. 5模型并排发表级三线表（build_regression_table）')
try:
    from analysis.panel_regression import build_regression_table, run_ols, run_panel_model
    dep, indeps = 'health_spending', ['did', 'income', 'pop_density', 'edu_level']
    r1 = run_ols(df_health, dep, indeps, cov_type='nonrobust'); r1['name'] = '(1) OLS'
    r2 = run_ols(df_health, dep, indeps, cov_type='HC3');       r2['name'] = '(2) OLS稳健SE'
    r3 = run_panel_model(df_health, dep, indeps, 'county_id', 'year', model_type='fe');   r3['name'] = '(3) 个体FE'
    r4 = run_panel_model(df_health, dep, indeps, 'county_id', 'year', model_type='twfe'); r4['name'] = '(4) 双向FE'
    r5 = run_panel_model(df_health, dep, indeps, 'county_id', 'year', model_type='re');   r5['name'] = '(5) 随机效应'
    tbl = build_regression_table([r1, r2, r3, r4, r5], key_vars=indeps)
    det = f'5模型并排三线表，变量：{indeps}\n{tbl.to_string(index=False)[:400]}'
    SECTIONS.append({'title': '30. 5模型并排三线表', 'content': det,
                     'table_headers': list(tbl.columns),
                     'table_rows': [[str(v) for v in row] for row in tbl.values.tolist()]})
    record('30. 5模型并排三线表', 'PASS', det)
except Exception as e:
    record('30. 5模型并排三线表', 'FAIL', traceback.format_exc()[:300])

# ─── T31: replace_key_variable（替换被解释变量）────────────────────────────
print('\n31. 替换被解释变量稳健性（replace_key_variable）')
try:
    from analysis.robustness import replace_key_variable
    res_alt = replace_key_variable(
        df_health, 'edu_level', ['did', 'income', 'pop_density'], 'did'
    )
    det = (f"替换被解释变量（health_spending → edu_level）\n"
           f"核心系数: {res_alt['coef']}{res_alt['stars']} SE={res_alt['se']} p={res_alt['pval']}\n"
           f"N={res_alt['n_obs']}  R²={res_alt['r2']}")
    SECTIONS.append({'title': '31. 替换被解释变量', 'content': det})
    record('31. 替换被解释变量稳健性', 'PASS', det)
except Exception as e:
    record('31. 替换被解释变量稳健性', 'FAIL', traceback.format_exc()[:300])

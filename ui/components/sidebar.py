    ...
    with st.sidebar:
        st.markdown(
            ...
            <div style="text-align:center; padding:1rem 0;">
                <h2 style="color:#2C3E50; margin:0;">📊 {t("sidebar_title")}</h2>
        ...
        st.caption(t("sidebar_current_step", n=current_step))
        ...
        if "df" in st.session_state and st.session_state["df"] is not None:
            ...
            st.success(t("sidebar_data_loaded", rows=len(df), cols=len(df.columns)))
        else:
        ...
        if analysis_results:
            st.caption(t("sidebar_analysis_done", n=len(analysis_results)))

        st.divider()
        st.markdown(
            f"""
            <div style="color:#95A5A6; font-size:0.75rem; text-align:center;">
                <p>{t("sidebar_version")}</p>
                <p>{t("sidebar_tagline")}</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
